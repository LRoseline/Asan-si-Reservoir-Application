...
public class RestCon {
    ...
    public VoirAPI VoirAPI(@PathVariable("no") int no) throws Exception {
        ...
        String URLComp = "http://apis.data.go.kr/B552149/reserviorWaterLevel/reservoirlevel/?serviceKey=" + this.setting.SettingLoad("datagokr").getValue() + "&pageNo=1&numOfRows=10&fac_code=" + api.getCode() + "&date_s=" + datesdf[0] + "&date_e=" + datesdf[1];
        DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
        DocumentBuilder documentBuilder = factory.newDocumentBuilder();
        Document doc = documentBuilder.parse(URLComp);
        Element element = doc.getDocumentElement();
        NodeList[] voir = { element.getElementsByTagName("fac_name"), element.getElementsByTagName("check_date"), element.getElementsByTagName("rate"), element.getElementsByTagName("water_level") };
        List<VoirDaily> appmedia = new ArrayList<>();
        for (int i = 0; i < 8; i++) {
    ...
    public VoirAPI VoirAPIGet(@PathVariable("no") int no) throws Exception {
        ...
        dates[0].add(5, -7);
        String[] datesdf = { sdf.format(dates[0].getTime()), sdf.format(dates[1].getTime()) };
        String URLComp = "http://apis.data.go.kr/B552149/reserviorWaterLevel/reservoirlevel/?serviceKey=" + this.setting.SettingLoad("datagokr") + "&pageNo=1&numOfRows=10&fac_code=" + vo.getCode() + "&date_s=" + datesdf[0] + "&date_e=" + datesdf[1];
